      ...
      >
        ...
        >
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 mb-6 pb-4 border-b border-[#adb3b7]/15">
            <button
              onClick={onClose}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/[0.06] hover:bg-white/[0.12] border border-[#adb3b7]/20 hover:border-[#adb3b7]/40 text-[#adb3b7] hover:text-white transition-all cursor-pointer group shadow-sm"
              aria-label="Back to projects"
            >
              <ArrowLeft className="w-4 h-4 transition-transform group-hover:-translate-x-1 text-[#ff7235]" />
              <span className="text-xs font-semibold">Back to Projects</span>
              <kbd className="hidden sm:inline-block px-1.5 py-0.5 text-[10px] font-mono text-[#adb3b7]/60 bg-white/5 rounded border border-[#adb3b7]/15 ml-1">
                ESC
              </kbd>
            </button>

            <div className="flex items-center gap-2.5 text-xs font-mono text-[#adb3b7]">
              <span className="font-mono text-xs text-[#ff7235] font-semibold px-2.5 py-1 rounded bg-[#ff7235]/10 border border-[#ff7235]/25">
                {`${project.number} // ${project.categoryLabel}`}
              </span>
              <span className="hidden md:inline text-[#adb3b7]/40">•</span>
              <span className="hidden md:inline font-mono text-[#adb3b7] text-xs">
          ...
          <p className="text-sm sm:text-base text-[#adb3b7] font-medium mb-6">
            {project.subtitle}
          </p>

          {/* Interactive Slide Deck Gallery Viewer */}
          <div className="relative w-full rounded-2xl overflow-hidden mb-6 border border-[#adb3b7]/20 bg-black shadow-2xl group">
            <div 
            ...
            >
              ...
              <div className="absolute top-3 right-3 flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/75 hover:bg-black text-white text-xs font-mono border border-white/20 opacity-0 group-hover:opacity-100 transition-opacity backdrop-blur-md z-10 pointer-events-none shadow-lg">
                <Maximize2 className="w-3.5 h-3.5 text-[#ff7235]" />
              ...
              <div className="absolute bottom-3 left-3 flex items-center gap-2 bg-black/80 backdrop-blur-md px-3 py-1 rounded-full border border-white/15 text-[11px] font-mono text-[#adb3b7]">
                <span className="text-[#ff7235] font-bold">SLIDE {activeSlideIndex + 1}</span>
                  ...
                  <button
                    ...
                    className={`relative flex-shrink-0 w-16 sm:w-20 aspect-[16/9] rounded-lg overflow-hidden border transition-all cursor-pointer ${
                      activeSlideIndex === idx
                        ? "border-[#ff7235] ring-2 ring-[#ff7235]/40 scale-105"
                        : "border-[#adb3b7]/20 opacity-50 hover:opacity-100"
                    }`}
                  >
                    <Image
                      src={slide}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
            {project.metrics.map((metric, idx) => (
              <div
                key={idx}
                className="p-5 sm:p-6 rounded-2xl bg-[#242424]/60 border border-[#adb3b7]/18 flex flex-col justify-between min-h-[130px] shadow-sm hover:border-[#ff7235]/30 transition-all"
              >
                {/* Row 1: Index & Status Badge */}
                <div className="flex items-center justify-between gap-2 mb-2">
                  <span className="text-[10px] font-mono text-[#adb3b7]/60 uppercase tracking-widest font-semibold">
                    METRIC 0{idx + 1}
                  </span>
                  {metric.change && (
                ...
                <div className="text-xs text-[#adb3b7] font-medium pt-2 border-t border-[#adb3b7]/12 line-clamp-1">
                  {metric.label}
                </div>
              </div>
            ))}
          ...
          {/* Deep Dive Breakdown */}
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-8 pb-8 border-b border-[#adb3b7]/15">
            <div className="space-y-3 p-5 rounded-2xl bg-[#242424]/60 border border-[#adb3b7]/15">
              <div className="flex items-center gap-2 text-xs font-semibold text-[#ff7235] uppercase tracking-wider font-mono">
                <TrendingUp className="w-4 h-4" />
                The Core Challenge &amp; Friction
              </div>
              <p className="text-[#adb3b7] leading-relaxed text-sm">
                {project.challenge}
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 mb-8">
            <div>
              <h4 className="text-xs font-mono uppercase tracking-wider text-[#adb3b7] mb-3 flex items-center gap-2">
                <Layers className="w-3.5 h-3.5 text-[#adb3b7]" />
                Key Deliverables &amp; Design Artefacts
              </h4>
              <ul className="space-y-2">
                {project.deliverables.map((item, idx) => (
                  <li key={idx} className="flex items-start gap-2 text-xs text-[#adb3b7]">
                    <CheckCircle2 className="w-4 h-4 text-[#ff7235] shrink-0 mt-0.5" />
                    <span>{item}</span>
                  </li>
                ))}
              </ul>
            ...
            <div>
              <h4 className="text-xs font-mono uppercase tracking-wider text-[#adb3b7] mb-3 flex items-center gap-2">
                <Wrench className="w-3.5 h-3.5 text-[#adb3b7]" />
              ...
              <div className="flex flex-wrap gap-2">
                {project.tools.map((tool, idx) => (
                  <span
                    key={idx}
                    className="px-3 py-1.5 rounded-lg bg-white/[0.04] border border-[#adb3b7]/18 text-xs font-medium text-[#f2f5f1]"
                  >
                    {tool}
                  </span>
                ))}
          ...
          <div className="flex flex-wrap items-center justify-between gap-4 pt-6 border-t border-[#adb3b7]/15 text-xs font-mono text-[#adb3b7]">
            <div>
              <span className="text-[#adb3b7]/70">ROLE:</span>{" "}
              <span className="text-[#f2f5f1] font-semibold">{project.role}</span>
            </div>
            <div className="text-[#adb3b7]/70">
            ...
            >
              ...
              >
                ...
                <div className="absolute top-5 left-5 sm:top-6 sm:left-7 z-30 flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-[#1f2024]/85 backdrop-blur-xl border border-[#adb3b7]/20 text-xs font-mono text-[#adb3b7] shadow-[0_8px_30px_rgba(0,0,0,0.6)] pointer-events-none">
                  ...
                  <span className="text-[#adb3b7]/50 hidden sm:inline">•</span>
                  <span className="text-[#adb3b7] hidden sm:inline">{project.title}</span>
                </div>
                ...
                >
                  ...
                  <X className="w-4 h-4 ml-0.5 text-[#adb3b7] group-hover:text-[#f2f5f1]" />
                </button>
